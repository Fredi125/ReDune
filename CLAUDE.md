...
- [x] **Irulan intro subtitles decoded** → `IRUL1..9.HSQ` are 8-bit subtitle text strips (IRUL1 = "A beginning...", readable via the 8-bit decode + IRULAN.HNM palette / grayscale). Extracted the 30-pair show/clear frame timeline (`IRULAN_SUBTITLE_FRAMES` in `lib/constants.py`, verified @0x22A58). Strips are viewable in the Sprites tab today; a frame-synced subtitle overlay on the Video tab is a ready follow-up (gamedata has IRUL1-9, a subset of the full ~30-line sequence).
